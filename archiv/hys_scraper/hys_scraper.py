...
class HYS_Scraper:
    def __init__(
        ...
        continue_feedback_scrape: bool = False, #QB added this for feedback connection error handling
        sleep_time: int = 1,
    ) -> None:
        ...
        self.continue_feedback_scrape = continue_feedback_scrape #QB added this
    ...
    def _scrape_pages(self) -> List[dict]:
        ...
        if self.continue_feedback_scrape == True:
            with open('feedback_backup.pkl', 'rb') as f:
        ...
        for page in range(start_page, n_pages): #QB added skipping of already scraped feedbacks
            ...
            ##QB added ConnectionError handling
            try:
                current = self._scrape_page(page, size=size)
                feedbacks += current["_embedded"]["feedback"]
            except Exception as e:
                print(f"\n***\nerror during feedback-scraping of page {page}\nsaved scraped feedbacks in feedback_backup.pkl")
                with open('feedback_backup.pkl', 'wb') as f:
